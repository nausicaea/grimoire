use std::{
    collections::HashMap, fmt::Display, net::{AddrParseError, IpAddr}, str::FromStr, time::Duration
};
...
use reqwest::{header::{HeaderMap, HOST}, redirect::Policy, ClientBuilder, Proxy};
use thiserror::Error;
            ...
            map.insert(
                ...
                group.map(|(_, value)| { 
                    ...
                    if header == reqwest::header::SET_COOKIE {
                        let mut cookie = Cookie::parse(utf8_value)
                            .expect("when parsing a cookie");
                        cookie.set_value("");
                        cookie.to_string()
                    } else {
...
impl Display for AnonymizedHttpHeader {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        let map_str = serde_json::to_string(&self.0)
            .map_err(|e| {
...
async fn main() -> anyhow::Result<()> {
    ...
    let mut target_stream =
        tokio_util::codec::FramedRead::new(tokio::io::stdin(), LinesCodec::new()).map(
            |line_result| {
                line_result.map_err(|e| Error::from(e)).and_then(|line| {
                    line.split_once(' ')
    ...
    while let Some(target) = target_stream.next().await {
        ...
        let http_request = client
            .head(format!("http://{target_fqdn}"))
            .build()?;

        if let Ok(http_response) = client.execute(http_request).await {
            println!("{target_fqdn} {target_ip} {} {} {}", http_response.url(), http_response.status().as_u16(), AnonymizedHttpHeader::from(http_response.headers()));
        }

        let https_request = client
            .head(format!("https://{target_fqdn}"))
            .build()?;

        if let Ok(https_response) = client.execute(https_request).await {
            println!("{target_fqdn} {target_ip} {} {} {}", https_response.url(), https_response.status().as_u16(), AnonymizedHttpHeader::from(https_response.headers()));
        }
